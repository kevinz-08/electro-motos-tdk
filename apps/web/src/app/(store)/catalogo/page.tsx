...
function LandingView({
  ...
  return (
    <div className="c-bg">
        ...
        <div className="text-center text-white max-w-3xl mx-auto px-4">
          ...
          <form method="GET" action="/catalogo" className="flex gap-2 max-w-xl mx-auto">
            <div className="relative flex-1">
              ...
              className="w-full bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl pl-11 pr-4 py-4 text-sm text-white placeholder-white/35 focus:outline-none focus:border-sky-300 transition-colors"
              />
            </div>
            <button
              type="submit"
              className="shrink-0 bg-sky-400 text-black px-7 py-4 rounded-xl text-sm font-bold hover:bg-sky-500 hover:text-white active:scale-95 transition-all"
            >
              Buscar
            </button>
          </form>
      ...
      {/* ── 4. TRUST STRIP ───────────────────────────────────────── */}
      <section className="border-t c-divider bg-black text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {TRUST.map((item) => (
              <div key={item.label} className="flex items-center gap-3">
                <span className="text-sky-300 shrink-0">{item.icon}</span>
      ...
      <section className="border-t border-white/10 bg-black">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="bg-white/5 border border-white/15 rounded-2xl p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div>
              <h3 className="text-lg font-bold text-white">¿No encuentras lo que buscas?</h3>
              ...
              <div className="hidden sm:flex items-center gap-2 flex-wrap">
                {activeChips.map((chip) => (
                  <Link
                    key={chip.label}
                    href={chip.removeUrl}
                    className="inline-flex items-center gap-1.5 bg-sky-50 border border-sky-200 text-sky-700 text-xs font-medium px-3 py-1.5 rounded-full hover:bg-sky-100 transition-colors"
                  >
                    {chip.label}
          ...
          <div className="sm:hidden flex flex-wrap gap-2 mb-5">
            {activeChips.map((chip) => (
              <Link
                key={chip.label}
                href={chip.removeUrl}
                className="inline-flex items-center gap-1.5 bg-sky-50 border border-sky-200 text-sky-700 text-xs font-medium px-3 py-1.5 rounded-full"
              >
                {chip.label}
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
