...
const buildBadges = (freeShippingThreshold: number) => [
  {
    icon: (
    ...
    label: freeShippingThreshold > 0 ? `Envío gratis desde ${formatCOP(freeShippingThreshold)}` : 'Envíos a toda Colombia',
    sub: 'A toda Colombia',
  },
  {
    icon: (
...
export async function TrustBadges() {
  ...
  const BADGES = buildBadges(freeShippingThreshold)
  return (
